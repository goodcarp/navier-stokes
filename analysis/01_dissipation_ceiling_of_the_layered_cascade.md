...
**Lean check (2026-09-08).** The exponent statements of this note were formalised in `lean/Cores/CeilingExponent.lean` (sorry-free, standard axioms). One hypothesis was missing in the prose: the supremum over `Q >= 1` of `beta/(2Q)` equals `beta/2` only for `beta >= 0` (for `beta < 0` the supremum is `0`); the note's `beta = 1/8` is positive, so nothing downstream changes. The equivalence `lambda^(alpha Q) < lambda^(beta/2) <-> alpha Q < beta/2` holds for every real `Q` when `lambda > 1`; `Q >= 1` is used only in the optimisation.
