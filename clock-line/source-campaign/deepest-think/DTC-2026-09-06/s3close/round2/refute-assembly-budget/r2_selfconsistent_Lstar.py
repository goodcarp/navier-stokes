...
# 1. the assembly's own L_* values, re-read from its a2/a4/a8 JSONs (read-only)
ASM = os.path.join(HERE, "..", "..", "assembly")
a2 = json.load(open(os.path.join(ASM, "a2_results.json")))
a4 = json.load(open(os.path.join(ASM, "a4_results.json")))
a8 = json.load(open(os.path.join(ASM, "a8_results.json")))
OUT["assembly_L_star_at_cstar"] = a2["L_star"]

# 2. eps at the assembly's L_* on the windows the theorem would need
probe = {}
for col in ["proved", "measured", "proved_sharpsens"]:
    Lh = a2["L_star"]["%s_eps<=0.5" % col]
    row = {"L_star_assembly": Lh}
    for r in [1.0, 1.05, 1.1, 1.25, 1.5]:
        row["eps_at_c/c*=%g" % r] = min(eps_N(Lh, 1, col, CSTAR*r, f) for f in F_SET)
    row["slack_best"] = slack(Lh, 1, col)
    probe[col] = row
OUT["probe_at_assembly_Lstar"] = probe

# 3. the self-consistent thresholds
sc = {}
for col, N, C_K in [("proved", 1, 1.0), ("measured", 1, 1.0), ("proved_sharpsens", 1, 1.0),
                    ("proved", 16, 1.0), ("measured", 16, 1.0),
                    ("proved", 16, 66.6622), ("measured", 16, 66.6622)]:
    t0 = time.time()
    Ls, info = Lstar_selfconsistent(N, col, C_K)
    key = "%s_N=%d_CK=%g" % (col, N, C_K)
    sc[key] = {"L_star_selfconsistent": Ls,
               "c_over_cstar_at_threshold": info[1] if info else None,
               "eps_at_threshold": info[2] if info else None,
               "seconds": time.time()-t0}
    print(key, sc[key], flush=True)
OUT["L_star_selfconsistent"] = sc
OUT["assembly_L_star_N16_a4"] = {k: v for k, v in a4["table"].items() if k.endswith("N=16") or k.endswith("N=1")}
OUT["assembly_a8_keys"] = list(a8.keys())

with open(os.path.join(HERE, "r2_results.json"), "w") as fh:
