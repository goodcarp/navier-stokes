...
| **Theorem.** In the axisymmetric sub-class of Fefferman's (C): on the axis, Type II, swirl required. | Proved; restated the same evening in the bare Clay class (strong-solution hypothesis deleted via an adapted comparison lemma and a new finite-dissipation proposition; one step enclosed via CKN). Refereed; 11 corrections; second-vendor check in progress. | [`theorems/02_…`](theorems/02_forced_axisymmetric_on_axis_typeII.md) |
