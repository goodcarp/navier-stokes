...
| **Theorem.** In the axisymmetric sub-class of Fefferman's (C): on the axis, Type II, swirl required. | Proved; restated the same evening in the bare Clay class (strong-solution hypothesis deleted via an adapted comparison lemma and a new finite-dissipation proposition; one step enclosed via CKN). Refereed; 11 corrections; the discharge confirmed blind by a second, independent line, which also sharpened the dissipation bound to an equality. | [`theorems/02_…`](theorems/02_forced_axisymmetric_on_axis_typeII.md) |
...
| **The log clock.** Vorticity doubling time ≤ c₂(1+ε)/(M log Re), c₂ = 1.649, matching the lower bound already on record. Would make a published log-free bound false as stated. | Written end to end; remaining items enumerated, two discharged the same day. | [`theorems/03_log_clock/`](theorems/03_log_clock/THEOREM_S3/THEOREM_S3.md) |
...
| **The release, scored.** The evening's forced Navier–Stokes paper and Lean certificate, and the unforced Euler paper, run against the checklist. | Every decisive item passes; no fatal or major defect found; the certificate covers Clay (C)/(D) verbatim and not the paper's distinctive clauses. 36 readers, one refuter per finding. | [`audit/02_…`](audit/02_openai_release_audit_2026-09-08.md) |
...
```
M · T_d  ≤  c₂ (1 + ε(L)) / (2L),      c₂ = 2 log(3/2) / κ_δ = 1.6489464
```
