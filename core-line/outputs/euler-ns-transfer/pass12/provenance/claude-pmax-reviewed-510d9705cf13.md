...
11. **The cost of fixing both is small.** `f` is nearly free in the proved column
    ...
    changes `L_*` by `a factor `1.0000116`, from `311511.943` to `311515.554``. Numbers in section B5.
...
set). Where `m(s) = 0` the same bound holds trivially, since `m >= 0` and `sigma >= 0`.
`m` is nonnegative and locally Lipschitz with `D^+ m <= gamma m + sigma` throughout, so the
standard comparison with the solution of `y' = gamma y + sigma`, `y(s_0) = m(s_0)`, dominates
it. Under (i) this gives the conclusion for `0 < s_0 <= s <= tau`; under (ii), for
`0 <= s_0 <= s <= tau`.
...
(c) split at `rho = 2 rho_0`: inside, `grad eta` is bounded by (a); outside, use the display;
`int Xi_9 rho^4 drho < infinity` because `9 > 5`. Nothing in (a), (b) or (c) uses P1' or P2',
so there is no circularity: (a) and (b) are inputs to P2', and (c) is used only in `u2`
section 6. ∎
...
`rho|eta_0^{(sigma)}| = M |F_sigma| B` and
`rho^2|grad eta_0^{(sigma)}| = M sqrt(F_sigma^2 A^2 + F_sigma'^2 B^2)`, so pointwise in
`(phi,u)`
```
     F_sigma^2 A^2 + F_sigma'^2 B^2  <=  ||F||_inf^2 A(u)^2 + ||F'||_inf^2 B(u)^2 ,
```
and the supremum over `u` of the right side is attained deep in the plateau, at `A = B = 1`,
where it equals `||F||_inf^2 + ||F'||_inf^2 = 1/sin^2 delta + cos^2 delta/sin^4 delta =
1/sin^4 delta = Gfrak_0^2`. (That the maximum over `u` sits at `A = B = 1` is a computation, not an assertion: the global
maximum of `||F||^2 A^2 + ||F'||^2 B^2` over the `u`-grid is `3445.1594359366854 = Gfrak_0^2`,
attained at `A^2 = B^2 = 1`, and restricted to `Theta < 0.99` it is `3386.1663916755992`.
`q2` runs the same maximisation over the exact convex hull of `{(A(u)^2, B(u)^2)}`, so no
`u` is missed.) The same argument
with `B <= 1` gives `sup rho|eta_0^{(sigma)}| <= ||F||_inf M = E_0 M`. `q2` confirms both at
four mollification scales:
```
...
`3.9318517` at once. What that costs in `kappa_delta` is not computed here.
...
**Dropping the sharp-edge cap `d <= f`, and shrinking `d` instead.** The cap `d <= f` is the
inner constraint for a sharp radial edge and is meaningless for the `tanh` ramp; without it
`d` is taper-limited at every `f` and the ball reaches further in, which makes `vartheta`
worse at small `f` and identical from `f = 1` up (where the cap was not binding anyway).
Halving `d` helps, but not nearly enough to rescue small `f`:

| `f` | `0.05` | `0.1` | `0.25` | `0.5` | `1` | `2` | `4` | `8` |
|---|---|---|---|---|---|---|---|---|
| `vartheta`, `d = d_max` | `6.6150` | `7.9678` | `10.483` | `11.515` | `6.8754` | `2.4235` | `0.055612` | `5.0751e-04` |
| `vartheta`, `d = d_max/2` | `1.9806` | `2.3618` | `4.8618` | `5.0286` | `2.9475` | `0.28040` | `4.9106e-03` | `4.4597e-05` |

So `f` has to move; `d` cannot be traded for it at small `f`. At `f = 2` a half-radius ball
does give `vartheta = 0.28040`, which is admissible, at the cost of a factor `2` in
`E_hess`'s `4 N E[W]/d` term and a larger one in `E_tail`. I did not run the budget on that
variant; the numbers of B5 are for ASSEMBLY's own `d` throughout, which is the conservative
...
`q4_budget_theta.py` imports `copies/t3_budget.py` and `copies/t2_gamma_CR.py` byte for byte
and edits nothing; it replaces only `assemble`, so that `eps_v` becomes Corollary V.5':

```
   eps_v(vartheta)  =  eps_bulk [ 1 + vt (9 + 2 sigma_z/sigma_y) ]  +  Q ( E_hess + E_4 + E_tail ) .
```

`vartheta(f)` is read from `q3_results.json`. A grid search returns a **lower** bound on a
supremum, so the budget is run at `SAFETY * vartheta`; `SAFETY = 2` is the headline and
`SAFETY = 1` and `4` are reported beside it.

**Control (L-14).** Driven with `vartheta = 0` the imported instrument returns

| column | `L_*` here | THEOREM_S3 section 4.5 |
|---|---|---|
| proved, `eps <= 1/2` | `311511.94343596586` | `311511.9` |
| proved, `C_a` measured, `eps <= 1/2` | `302704.27710465185` | `302704.3` |
| measured, `eps <= 1/2` | `422.43343046493067` | `422.4` |
| proved, `eps <= 0.1` | `1056242.9239566554` | `1056242.9` |

so the instrument is the one THEOREM_S3 used, unmodified.

**Admissibility.** `(H3'_vartheta)` needs `vartheta < 1`. At `SAFETY = 2`:

| `f` | `0.05` | `0.1` | `0.25` | `0.5` | `1` | `2` | `4` | `8` | `16` | `32` |
|---|---|---|---|---|---|---|---|---|---|---|
| `2 vartheta` | `2.601` | `2.919` | `10.15` | `19.28` | `13.75` | `4.847` | `0.1112` | `1.015e-03` | `6.264e-06` | `3.107e-08` |
| admissible | no | no | no | no | no | no | **yes** | yes | yes | yes |

The whole effect of (H3-V) on the budget is this row of `no`s. Six of the ten `f` in
THEOREM_S3's own grid, including its minimiser, are struck.

**What it costs.** At `L = 3e+05`, `f = 4`, proved column:

| | `vartheta = 0` | `vartheta = 0.11122` |
|---|---|---|
| `eps_bulk` | `5.0701e-09` | `5.0701e-09` |
| `E_vartheta` (new) | `0` | `1.0350e-08` |
| `E_hess` | `7.0818e-08` | `x Q` |
| `E_4` | `1.3498e-11` | `x Q` |
| `E_tail` | `0.0` | `0.0` |
| `eps_v` | `7.5902e-08` | `1.0398e-07` |

a factor `1.3699` on a term that sits more than six orders of magnitude below the binding one
(`eps_a = 0.34529`, through `eps_T'`; the ratio is `3.3e+06`). At `L = 1e+05` the factor is `1.2912` and at
`L = 1e+06` it is `1.5972`.

**`L_*` and `log Lambda_*`.**

| column, target | `vartheta = 0` | `SAFETY = 1` | `SAFETY = 2` | `SAFETY = 4` | best `f` |
|---|---|---|---|---|---|
| proved, `eps <= 1/2` | `311511.943` | `311515.538` | **`311515.554`** | `311515.601` | `4` |
| proved, `eps <= 0.1` | `1056242.924` | `1056258.505` | `1056258.558` | `1056258.717` | `4` |
| proved `C_a` meas., `eps <= 1/2` | `302704.277` | `302707.846` | `302707.877` | `302707.922` | `4` |
| measured, `eps <= 1/2` | `422.433` | `424.305` | `424.528` | `425.069` | `4` |

```
   L_*        311511.943  ->  311515.554        ratio 1.0000116
   log Lam_*  623026.810  ->  623034.032        (= 2 L_* + 2.923485894768454)
   best f            0.1  ->  4
```

**The reading.** The change in `L_*` is `+3.61`, one part in `86000`. That is the honest
number and it is also the least interesting one, because the viscous column was already
numerically irrelevant (THEOREM_S3 finding 3). What (H3-V)'s repair actually costs is not a
number in `L_*` but a **constraint on the family**: the theorem must be stated at `f >= 4`,
where the tracked ball is inside the plateau and `Theta(rho_*) = 0.999981`, and the budget's
own `f`-minimisation must be told about it. The three columns at `SAFETY = 1, 2, 4` differ in
the sixth digit, so the answer does not depend on how conservatively `vartheta` is bounded;
it depends only on which `f` are admissible at all.

Note also that `eps` is increasing in `f` at fixed `L` (through `ell_loss`'s `log(1+f)/L`), so
the minimiser is always the **smallest admissible** `f`, which is `4` at every `SAFETY` tested.

---

## C. FINDINGS

**F1. (P-max) closes, and the referee's first attack point closes with it.**
refute-u2 section 7 item 1 asks whether the supremum in P1 and P2 can escape to infinity, and
calls it the place to spend the first day. It cannot, and no property of the datum's tails is
needed to say so: the barrier `h = e^{Ks}(rho^2+1)^{beta/2}` of Lemma E case (ii) catches any
`Z` growing more slowly than `rho^beta` with `beta <= 3`, and `V_eps` grows like `rho` while
`W_eps` grows like `rho^2`. The `beta <= 3` ceiling is the same `k(k-n+2) <= 0` ceiling at
`n = 5` that makes the weights work at all.

**F2. Two of the chain's hypotheses become lemmas.** refute-u2 MINOR-4 lists
`rho|eta| -> 0`, `rho^2|grad eta| -> 0` and `grad eta in L^1` as used but unstated. Lemma F
proves the decay with an explicit rate `Lambda_9 = 9 Gamma-bar + 99 nu/R^2` (the second term
is below `1e-300` at every `L` in play, so the constant is `e^{9 c_G}`), and Corollary F'(c)
proves the `L^1` with an explicit bound. `grad eta in L^1` is used in **exactly one place** in
the whole chain, `u2` section 6's `grad a = K * grad_5 eta`, and in neither P1 nor P2.

**F3. The regularisation is exact, and mollifying the weight is free.** Replacing `rho` by
`R_eps` produces (A.2) and (A.3) with **no** error term and with zeroth-order coefficients
`-(2 rho^2 + 5 eps^2)/R_eps^4` and `-(2 rho^2 + 10 eps^2)/R_eps^4`, both of the good sign for
every `eps`; and it makes the first-order coefficient `2k x/R_eps^2` bounded, which is what
removes the singularity of `u2` section 5's argument at the origin. The only inexactness in
the whole of Part A is the single `Gamma eps` in (A.5), which is the price of shifting
`sqrt(|g|^2+eps^2)` down by `eps` so that `W_eps` vanishes at infinity.

**F4. `Gfrak_0 = E_0^2 = 1/sin^2 delta`, exactly.**
`E_0 = 1/sin delta = 7.661297575540389` and `Gfrak_0 = 58.69548054098104`, both closed forms,
both attained at the taper kinks, and `E_0` is the campaign's own `s`. The plateau identity
`F^2 + F'^2 = 1/sin^4 phi` is the whole derivation. THEOREM_S3's `58.695476` and refute-u2's
`58.6948961` are this number, low by `7.2e-08` and `1.0e-05`, and the discrepancy is a
floating-point cancellation near the poles of the angular grid, not a difference of datum.
Since THEOREM_S3 finding 1 identifies `Gfrak_0` as the binding **datum** quantity, this says
the entire datum dependence of `Ghat`, `C''`, `C_R` and `L_*` in the proved column is
`sin^{-2} delta` and nothing else. Widening `delta` from `7.5` to `15` degrees divides
`Gfrak_0` by `3.9318517` at a stroke; what it costs in `kappa_delta` is not computed here and is
the obvious next measurement.

**F5. Brief-layer correction (L-17).** The brief expects `E_0 ~ 4.09 M`. That is
`||eta_0||_inf rho_0/M = 4.093544431740673`, refute-u2 MINOR-2's functional, computed here for
the theorem's datum (refute-u2 reports `4.093173` for (D-C); the two profiles differ only in
`sup|F|`, `7.66130` against `7.66060`). `E_0 = sup rho|eta_0|/M = 7.661297575540389`. Both are
reported; they are not the same object and the brief conflates them.

**F6. A negative result, so it is not re-attempted.** P2's rate `Gamma = ||grad_5 b||_op` can
be replaced by the sharp `-Lambda_min(sym grad_5 b)`. On the reference strain
`b = a diag(1,1,1,1,-2)` the two are equal, both `2a`, while `Gamma_rad = a`. The sharpening
gains a factor `1.000000` in the regime that dominates. refute-u2 MAJOR-1's `4.18x` of
headroom is in *where the weighted supremum sits*, not in the rate.

**F7. M5 (H5-Lip) is discharged for P1 and P2, at zero cost in either constant.** Mollifying
the angular profile raises neither `E_0` nor `Gfrak_0` (Proposition 3(b), and four numerical
scales confirm it), and the classical regularity M5 asks for is Lemma A. So the trade M5
describes, going to the fully-`tanh` datum at a cost of `2.8057` in `Gfrak_0` and of
`kappa_delta` from `0.49782244` to `0.47473436`, is **not forced** by P1 and P2. Whatever else
M5 covers, its P-max half is closed and the theorem's own datum may be kept.

**F8. ASSEMBLY BLOCK 5's "one-line change to Step 2's Taylor remainder" is not one line.**
Step 2 contracts the Hessian against an **anisotropic** `C_tau` with
`sigma_z/sigma_y = 3.5130747`, so a Laplacian clause leaves `d_z^2(eta_0 - eta_P)` free with a
coefficient `2.5130747`; and Step 2's remainder is a fourth-derivative statement that no value
or Laplacian bound implies. The correct hypothesis is the scale-free family `k = 0,...,4`, and
the binding order is `k = 4` at every `f >= 0.25`. This is the clause the proposed repair does
not contain.

**F9. (H3) and (H3') are vacuous at the budget's own `f`.** `vartheta(f = 0.1) = 1.4593`,
`vartheta(f = 1) = 6.87194`, `vartheta(f = 2) = 2.42094`; the hypothesis needs `vartheta < 1`
and first has content at `f = 4`, where `vartheta = 0.055612`. The mechanism is a single line:
on the ball the angular profile is exactly `1`, so the whole deviation is the radial ramp,
`psi_in(rho) = e^2/(e^2 + (rho/rho_0)^8)`, and when `d` is taper-limited
`rho_min = (1 - sin(phi_0 - delta)) rho_* = 0.61732 rho_*`, so `vartheta` falls like `f^{-8}`.

**F10. A second defect sits at the same place and is not on the modulo list.** At the budget's
minimiser `f = 0.1` the tracked material point sits at `u_* = 0.0953`, below the ramp's
half-height `eps_r = 0.25`, and carries `Theta(rho_*) = 0.224868` of the nominal amplitude
`M`. `ell_loss(f)` charges `log(1+f)` for the inset and a flat `eps_r` for the outer ramp and
never asks whether the tracked point is in the plateau. `Theta(rho_*)` first exceeds `0.999`
at `f = 2`. I am not adding a modulo item for this: it is a bookkeeping question about
`ell_loss` for the `tanh` datum, it points the same way as (H3-V), and the same move fixes
both.

**F11. The price of closing (H3-V) is `1.16e-05` of `L_*`, and a constraint on the family.**
`L_* : 311511.943 -> 311515.554` (proved, `eps <= 1/2`, `SAFETY = 2`),
`log Lambda_* : 623026.810 -> 623034.032`, best `f : 0.1 -> 4`. The `SAFETY = 1, 2, 4` columns
agree to six digits, so the answer does not depend on how conservatively `vartheta` is
bounded. What actually changes is that the theorem must be stated at `f >= 4`.

**What this note does NOT close.** M3 (V-strain), M4 (lam-cap), M6 (H\*), M7 (Theta-tail) and
M8 (`(Gamma-off)`'s existence threshold) are untouched. refute-u2's MAJOR-1 (the `4.18x` of
weight headroom) and MAJOR-2 (the datum scope of `C''`) are untouched. Lemma A cites interior
parabolic Schauder rather than proving it. **FL-000 stands.**
...
```
201 CHECKS, 201 PASS, 0 FAIL   (document tokens scanned: 1933)
```

`check_constants.py` does two jobs. It rebuilds every quantity this note leans on from the
mathematics, in the gate file itself, and compares it with what the four scripts stored: the
two closed forms from `1/sin delta`, the plateau identity `F^2 + F'^2 = 1/sin^4 phi` at ten
angles, the factorisation of `||eta_0||_inf`, the `L`-stability of the tail and TV constants,
the mollification monotonicity, the maximisation behind Proposition 3(b) recomputed from
`Theta` here, `vartheta_0 = psi(rho_* - d)` at every `f`, `Theta(rho_*)`, ASSEMBLY's `d` rule,
`Corollary V.5'`'s `eps_v` formula rebuilt from the stored pieces at every row of the term
table, and the control that the imported instrument reproduces THEOREM_S3's published `L_*`.
No check compares a stored literal with itself. Then it audits PROOF.md token by token:
every numeric literal is matched against the pool of values the scripts produced, to the
precision at which it is printed, or against the `FOREIGN` dictionary of numbers quoted from
other seats with their origins, or against the whitelist of structural integers.
...
consistent. It does not check that Lemma A cites the right theorem, that Lemma E's Dini step
is correctly quoted, that (H3'_vartheta) is the right hypothesis, or that the modulo list is
complete. Section A8 and section C are for that.
...
| `check_constants.py` / `gate.log` | the gate and the document audit, and its output |
