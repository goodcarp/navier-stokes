...
Updated 8 September 2026, after the twelfth transfer pass.
...
The [twelfth pass](pass12/REPORT.md) adds an actual-matrix reference with
an H4-compatible local deformation estimate, a separately audited repair
of Claude’s gradient-decay and initial-trace passage under explicit
hypotheses, and a compact potential endpoint reconstruction. The new
reconstruction is C6/H7, exactly divergence-free and axis regular; it is
not an NS time-slab validation or an all-order smooth force.

A sampled least-squares fit concealed severe between-node/core artifacts.
The corrected continuous L2 projection removes that observed failure.
Its two radial trial spaces retain positive numerical energy/core margins,
but differ in the core margin by about 4.85e-5. Localization, higher
spatial derivatives, full time-slab residuals and pressure tails remain
unbounded. No stored numerical margin has become an actual NS certificate.
