...
## Immediate target after the twelfth pass
...
The [pass12 continuous potential reconstruction](pass12/REPORT.md) now
provides an H7 whole-space endpoint approximation. Extend it to a full
time slab and control its higher derivatives and projected residual. Use
the continuous velocity Gram metric; node-only fitting can hide severe
core and energy artifacts. Preserve the exact initial datum, all phases,
join terms and inherited fields. The radial comparison and localization
change have not been enclosed in H4.

Use the [actual-matrix reference](pass12/c4-adapted-reference.md), especially
its H4-compatible mixed-power bound, to test core deformation after an
actual H4 error radius is available. H4 alone does not bound the actual
third derivative in L-infinity; the mixed-power estimate avoids that
invalid upgrade. The outer receiver still needs full matrix/mean coupling.
