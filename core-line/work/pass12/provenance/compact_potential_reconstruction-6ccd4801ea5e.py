...
This is a smooth whole-space APPROXIMATION, not a validated NS solution.
...
PRIOR = Path(__file__).resolve().parents[1] / 'pass11'
...
def fit_mode(g,delta_hat,j,intervals=96):
    ...
    m=int(g.modes[j]);basis=RadialBasis(g.R,intervals,4.,m)
...
def run(args):
    ...
    report={'status':'COMPLETED SMOOTH WHOLE-SPACE ENDPOINT RECONSTRUCTION; NOT NS VALIDATION',
